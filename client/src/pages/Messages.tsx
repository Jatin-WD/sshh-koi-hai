...
export default function Messages() {
  ...
  function handleError(value: unknown) {
    const message = value instanceof Error ? value.message : "Chat action failed";
    if (message.toLowerCase().includes("membership")) setMembershipRequired(true);
    else setError(message);
  }
  async function loadConversations() {
    try { const data = await api<{ conversations: Conversation[] }>("/conversations"); setConversations(data.conversations); if (!selectedId && data.conversations[0]) setSelectedId(data.conversations[0].id); }
    catch (value) { handleError(value); } finally { setLoading(false); }
  }
  async function loadMessages(id: string) {
    try { const data = await api<{ messages: Message[] }>(`/conversations/${id}/messages?page=1&pageSize=50`); setMessages(data.messages); }
  ...
  useEffect(() => {
    ...
    socket.on("disconnect", () => setStatus("Offline - reconnecting"));
    socket.on("connect_error", () => setStatus("Unable to connect privately"));
