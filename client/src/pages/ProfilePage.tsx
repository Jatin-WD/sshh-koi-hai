...
export default function ProfilePage() {
  ...
  const suggestions = [!profile.profileImages.length && "Add a profile photo", !profile.bio && "Write a short introduction", !profile.interests.length && "Add a few interests", !profile.relationshipIntent && "Share what you are hoping to find", !profile.languages.length && "Add the languages you speak"].filter(Boolean) as string[];
  ...
  async function deleteAccount() {
    ...
    catch (e) { setError(e instanceof Error ? e.message : "Unable to delete account"); setDeleting(false); }
  }
  ...
  return <section className="mx-auto max-w-6xl px-6 py-12">
    ...
    <div className="grid gap-8 lg:grid-cols-[1.35fr_.65fr]">
      ...
      <aside className="space-y-6">
        {suggestions.length > 0 && <div className="rounded-3xl border border-gold/40 bg-gold/10 p-7"><p className="text-xs uppercase tracking-[0.25em] text-burgundy/65">A gentle next step</p><h2 className="mt-3 font-display text-2xl">Make your introduction easier.</h2><ul className="mt-4 space-y-2 text-sm leading-6 text-charcoal/70">{suggestions.slice(0, 3).map((suggestion) => <li key={suggestion}>• {suggestion}</li>)}</ul><p className="mt-4 text-xs leading-5 text-charcoal/50">Add only what feels comfortable. Your profile does not need to be perfect to be yours.</p></div>}
