...
export default function NotificationBell() {
  ...
  async function markRead(id: string) {
    ...
    if (!item || item.readAt || saving) return;
    setSaving(true); setError("");
    try {
      await api(`/notifications/${id}/read`, { method: "PATCH" });
      setItems((current) => current.map((notice) => notice.id === id ? { ...notice, readAt: new Date().toISOString() } : notice));
      setUnread((current) => Math.max(0, current - 1));
    } catch (value) { setError(value instanceof Error ? value.message : "Unable to update notification"); }
