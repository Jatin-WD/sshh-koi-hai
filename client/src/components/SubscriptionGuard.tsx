...
export default function SubscriptionGuard() {
  ...
  useEffect(() => { setProfileCheck(null); if (!user || !isMemberAppRoute) { setProfileCheck(true); return; } api<{ completion: number }>("/profile/me").then((profile) => setProfileCheck(profile.completion >= 45)).catch(() => setProfileCheck(true)); }, [user, location.pathname, isMemberAppRoute]);
