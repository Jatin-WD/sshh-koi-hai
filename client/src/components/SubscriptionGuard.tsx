...
export default function SubscriptionGuard() {
  ...
  useEffect(() => { setProfileCheck(null); setProfileError(""); if (!user || !isMemberAppRoute) { setProfileCheck(true); return; } api<{ completion: number }>("/profile/me").then((profile) => setProfileCheck(profile.completion >= 45)).catch((value) => { setProfileCheck(null); setProfileError(value instanceof Error ? value.message : "We could not load your profile right now."); }); }, [user, location.pathname, isMemberAppRoute, retry]);
  if (authLoading || (user && allowed === null) || (profileCheck === null && !profileError)) return <div className="p-10 text-center text-charcoal/60">{isMemberAppRoute ? "Preparing your private space..." : "Checking membership..."}</div>;
