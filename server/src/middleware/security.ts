...
export const trafficController: RequestHandler = (req, res, next) => {
  ...
  if (req.path === "/api" || req.path.startsWith("/api/health") || req.path.startsWith("/socket.io")) return next();
...
export const sharedTrafficLimit: RequestHandler = (req, res, next) => {
  if (!sharedRedis || req.path === "/api" || req.path.startsWith("/api/health") || req.path.startsWith("/socket.io")) return next();
