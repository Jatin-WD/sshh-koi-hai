...
export const requireTrustedOrigin: RequestHandler = (req, _res, next) => {
  ...
  const trusted = origin ? corsOrigins.includes(origin) : corsOrigins.some((allowed) => referer?.startsWith(`${allowed}/`));
  return trusted ? next() : next(new AppError("Request origin is not allowed", 403, "CSRF_ORIGIN_REJECTED"));
};
