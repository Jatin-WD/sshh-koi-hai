...
import { requireMessagingMembership } from "../lib/membership.js";
...
router.get("/", requireMessagingMembership, async (req, res, next) => {
  try {
    ...
    for (const conversation of conversations) {
      ...
      try { await assertChatEligibility(userId, other.id); } catch { continue; }
      const unreadCount = await prisma.message.count({ where: { conversationId: conversation.id, senderId: { not: userId }, readAt: null } });
      visible.push({ id: conversation.id, matchId: conversation.matchId, lastMessageAt: conversation.lastMessageAt, unreadCount, latestMessage: conversation.messages[0] ? serializeMessage(conversation.messages[0]) : null, participant: publicProfile(other, other.profile) });
    }
    return sendSuccess(res, { conversations: visible });
  } catch (error) { return next(error); }
});

router.get("/:conversationId/messages", requireMessagingMembership, async (req, res, next) => {
  try {
    ...
    const input = paging.parse(req.query); const skip = (input.page - 1) * input.pageSize;
    const messages = await prisma.message.findMany({ where: { conversationId }, orderBy: { createdAt: "desc" }, skip, take: input.pageSize });
    await prisma.message.updateMany({ where: { conversationId, senderId: { not: req.authUser!.id }, deliveredAt: null }, data: { deliveredAt: new Date() } });
    return sendSuccess(res, { messages: messages.reverse().map(serializeMessage), pagination: { page: input.page, pageSize: input.pageSize, hasMore: messages.length === input.pageSize } });
  } catch (error) { return next(error); }
});
