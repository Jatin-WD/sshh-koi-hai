...
router.put("/me", requireAuth, async (req, res, next) => {
  try {
    ...
    return sendSuccess(res, { profile: ownProfile(updatedUser, updatedProfile), completion, visibilityNotice });
...
async function loadOwnProfile(userId: string) { const user = await prisma.user.findUnique({ where: { id: userId }, include: { profile: true } }); if (!user?.profile) throw new AppError("Profile not found", 404, "PROFILE_NOT_FOUND"); return { profile: ownProfile(user, user.profile), completion: profileCompletion(user, user.profile), account: { emailVerified: user.isEmailVerified, status: user.status } }; }
function ownProfile(user: Parameters<typeof publicProfile>[0], profile: Parameters<typeof publicProfile>[1]) { return { ...publicProfile(user, profile), genderPreference: profile.genderPreference, agePreferenceMin: profile.agePreferenceMin, agePreferenceMax: profile.agePreferenceMax, locationPreference: profile.locationPreference, showOnlineStatus: profile.showOnlineStatus, showCity: profile.showCity, allowInterests: profile.allowInterests }; }
